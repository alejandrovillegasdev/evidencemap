	...
	$.getJSON('./src/data/csvData.json', function(csvData) {
		$.getJSON('./src/data/filters.json', function(filters) {
			$.getJSON('./src/data/referenceData.json', function(referenceData) {
				...
				const segmentAttributes = [
					{
						attribute: {
							...
							AttributeName: 'High quality primary studies',
					...
					},
					{
						attribute: {
							...
							AttributeSetDescription: '',
							AttributeType: 'Selectable (show checkbox)',
							AttributeName: 'High quality secondary study',
							AttributeDescription: ''
						},
						color: '#fab852'
					},
					{
						attribute: {
							...
							AttributeSetDescription: '',
							AttributeType: 'Selectable (show checkbox)',
							AttributeName: 'Low and medium primary studies',
							AttributeDescription: ''
						},
						color: '#ffa500'
					},
				...
				const checkboxCheckedSvg =
					'<svg id="checked" fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">' +
					'<path d="M0 0h24v24H0z" fill="none"/>' +
					'<path d="M19 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.11 0 2-.9 2-2V5c0-1.1-.89-2-2-2zm-9 14l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>' +
					'</svg>';
				const checkboxUncheckedSvg =
					'<svg id="unchecked" fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">' +
					'<path d="M19 5v14H5V5h14m0-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"/>' +
					'<path d="M0 0h24v24H0z" fill="none"/>' +
					'</svg>';
				const checkboxIndeterminateSvg =
					'<svg id="indeterminate" fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">' +
					'<defs>' +
				...
				const radioCheckedSvg =
					...
					'<path d="M0 0h24v24H0z" fill="none"/>' +
					'</svg>';
				const radioUncheckedSvg =
					'<svg id="unchecked" fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">' +
					'<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z"/>' +
					'<path d="M0 0h24v24H0z" fill="none"/>' +
					'</svg>';
				const arrowUpSvg =
					'<svg class="btnRowCollapse" id="arrowUp" fill="#000000" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">' +
					'<path d="M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6z"/>' +
					'<path d="M0 0h24v24H0z" fill="none"/>' +
					'</svg>';
				const arrowDownSvg =
					'<svg class="btnRowCollapse" id="arrowDown" fill="#000000" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">' +
					'<path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/>' +
					'<path fill="none" d="M0 0h24v24H0V0z"/>' +
					'</svg>';
				const arrowLeftSvg =
					'<svg class="btnColCollapse" id="arrowLeft" fill="#000000" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">' +
					'<path d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6 1.41-1.41z"/>' +
					'<path fill="none" d="M0 0h24v24H0V0z"/>' +
					'</svg>';
				const arrowRightSvg =
					'<svg class="btnColCollapse" id="arrowRight" fill="#000000" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">' +
				...
				function buildTableColHead() {
					...
					for (let i = 0; i < csvData.totalColDepth; i++) {
						...
						for (let j = 0; j < row.length; j++) {
							...
							tableHeadHtml +=
								'<th class="' + cellClass + '" colspan="' + colSpan + '"' + style + data + '>';
							tableHeadHtml += '<div>';
         ...
         */
				function GetFilteredReferences(rowIdList, colIdList) {
					const references = [];
					const refMatches = referenceData.filter((reference) => {
						...
						if (rowIdList.length > 0 && colIdList.length > 0) {
							for (const rowId of rowIdList) {
								for (const colId of colIdList) {
									const codeTest = reference.Codes.filter(
										(code) => code.AttributeId === rowId || code.AttributeId === colId
									);
									if (codeTest.length >= 2) return true;
								}
							}
						} else {
							for (const code of reference.Codes) {
								if (rowIdList.length === 0) {
									// No row intersections to check for.
									if (colIdList.indexOf(code.AttributeId) >= 0) return true;
								} else if (colIdList.length === 0) {
									// No column intersections to check for.
					...
					for (const reference of refMatches) {
						...
						for (const parentFilter of parentFilters) {
							...
							for (const childFilter of parentFilter.children.filter((item) => item.checked)) {
								const childFilterMatch =
									reference.Codes.filter((code) => code.AttributeId === childFilter.id).length > 0;
    ...
    <rect width="100%" height="100%" fill="url(#grid)" />`;
					...
					$('.cell', $pivotBody).each((index, cell) => {
						...
						for (const reference of references) {
							for (const count of counts) {
								...
								const countMatched =
									reference.Codes.filter((code) => code.AttributeId === count.id).length > 0;
								if (countMatched) {
									count.count += 1;
									totalCount += 1;
								}
						...
						for (const count of counts) {
							count.width =
								totalCount === 0 || count.count === 0 ? 0 : Math.round(count.count / totalCount * 100);
         ...
         */
				function updateFilterParentNodeCheckedState($target) {
					...
					if (klass.length > 0) {
						...
						$target.removeClass($target.attr('class')).addClass(klass);
					}
				}
         ...
         */
				function updateFilterNodeCheckedState($target, newKlass = '') {
					...
					if ($target.is('li')) {
						...
						$target.removeClass(klass).addClass(newKlass);
         ...
         */
				function updateViewStyle($filters, $target) {
					...
					$('.style-wrapper li', $filters).removeClass('checked').addClass('unchecked');
				...
				function createSettingsPanel() {
					...
					function handleUpdate() {
						...
						setTimeout(() => {
							...
							$filters.removeClass('open');
							$veil.removeClass('open');
							$update.removeClass('busy').addClass('disabled');
							$update.off('click');
						}, 300);
					}
				}
         ...
         */
				function createFiltersPanel() {
					let filterHtml = '<div class="filter-wrapper">' + '<h2>Filters</h2>';
         ...
         */
				function createStylesPanel() {
					...
					if (segmentAttributes.length !== 0) {
						styleHtml +=
							'<li class="unchecked" data-id="donut">' +
							radioCheckedSvg +
							radioUncheckedSvg +
							' <span>Donut-map</span>' +
							'</li>';
					...
					$('.cell')
						.hover(
							(e) => {
								...
								$(tooltipHtml).appendTo('body').fadeIn('fast');
							},
							(e) => {
								// Hover out.
								$('.tooltip').remove();
         ...
         */
				function selectReference(refId) {
					...
					if (reference.Abstract !== undefined && reference.Abstract.length > 0) {
						...
						summaryAttribute.length > 0 &&
						reference[summaryAttribute] !== undefined &&
						reference[summaryAttribute].length > 0
					) {
         ...
         */
				function updateReferenceReader(references) {
					...
					for (const ref of references) {
						...
						refsHtml +=
							'<li class="refMenuItem" data-refid="' +
							ref.ItemId +
							'">' +
							'<div class="title">' +
							ref.Title +
         ...
         */
				function buildReaderFilter(rowIdList, colIdList) {
					...
					let html =
						'<div class="reader-filter">' +
						'<ul>' +
						'<li class="' +
						colParent +
						'" data-parent="true" title="' +
						csvData.rows[0][0].title +
						'">' +
						checkboxCheckedSvg +
						checkboxUncheckedSvg +
						checkboxIndeterminateSvg +
					...
					html +=
						...
						rowParent +
						'" data-parent="true" title="' +
						csvData.rows[csvData.totalColDepth][0].title +
						'">' +
						checkboxCheckedSvg +
						checkboxUncheckedSvg +
						checkboxIndeterminateSvg +
					...
					$('.reader-filter li').on('click', (e) => {
						...
						if (isParent) {
							$target.next().children().each((index, child) => {
								$(child).removeClass('checked').removeClass('unchecked').addClass(newKlass);
							});
						} else {
							const parent = $target.parent().prev();
							$(parent).removeClass('checked').removeClass('unchecked').addClass('indeterminate');
						}
				...
				function handleAboutClick() {
					$('.menu-about').on('click', (e) => {
						let aboutHtml =
							...
							'</div>' +
							'<div class="content">' +
							'<div class="read"> Hola Soy Alejo</div>' +
							'</div>';
         ...
         */
				function adjustTable() {
					...
					let sideHeadCssHeight =
						$window.height() - $topHead.height() - $footer.height() - $menu.height() - 48;
